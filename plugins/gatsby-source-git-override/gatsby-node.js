...
exports.sourceNodes = async (
  ...
  const localPath =
    local ||
    require("path").join(programDir, `.cache`, `gatsby-source-git`, name)
        ...
        return repo
          ...
          .then(log => {
            ...
            if (overrideModifiedTime) {
              const lastUpdate = new Date(Date.parse(date)).toISOString()
              fileNode.birthTime = new Date(
                Date.parse(log.all[log.all.length - 1]?.date)
              ).toISOString()

              fileNode.modifiedTime = lastUpdate
            }
